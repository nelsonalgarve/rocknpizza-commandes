...
export default function CommandeCard({ commande, onUpdate, onPrint }: Props) {
  ...
  return (
    <div
      key={commande.id}
      className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 shadow-lg border border-gray-200 dark:border-gray-600 w-full sm:w-[48%] lg:w-[32%]"
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {new Date(commande.date_created).toLocaleString('fr-FR')}
          </p>
        ...
        <span className={`text-sm font-bold px-4 py-1.5 rounded-full shadow-md border ${
      ...
      <div className="divide-y divide-gray-200 text-base text-gray-700 dark:text-gray-300">
        {commande.line_items.map((item, idx) => (
          <div key={idx} className="py-2 flex justify-between">
            ...
            <span className="font-semibold text-gray-800 dark:text-white">{totalTTC(item)} €</span>
          </div>
        ))}
      </div>

      <div className="flex justify-between items-center text-sm text-gray-600 border-t pt-3 mt-3">
        <span className="font-bold text-lg text-gray-800 dark:text-white">Total : {commande.total} €</span>
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        {commande.status === 'processing' && (
          <button
            className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-1.5 rounded-lg shadow-md"
        ...
        {commande.status === 'preparation' && (
          <button
            className="bg-green-600 hover:bg-green-700 text-white px-4 py-1.5 rounded-lg shadow-md"
        ...
        {commande.status === 'completed' && (
          <button
            className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-1.5 rounded-lg shadow-md"
        ...
        <button
          className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-1.5 rounded-lg shadow-md"
